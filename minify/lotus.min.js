...
Lotus.Context = function(config, params) {
    ...
    this.componentMap = new Lotus.ComponentMap(this);
    this.commandMap = new Lotus.CommandMap(this);
    this.injector = new Lotus.Injector(this);
...
Lotus.CommandMap.prototype.addCommand = function(eventType, handler, functionName, useSingleton) {
    ...
    this.eventFunctionMap[eventType].push({
        eventType: eventType,
        handler: handler,
...
Lotus.ComponentList.prototype.removeItemAt = function(index) {
...
Lotus.ComponentMap.prototype.addComponent = function(tagInstance, functionConstructor) {
    ...
    if (src !== null && src !== undefined) {
        ...
        httpService.send("GET", src, null, "text/html", "html", true);
...
Lotus.MediatorMap.prototype.add = function(tagName, mediatorConstructor, useSingleton) {
    ...
    this.tagConstructorMap[tagName] = {
        useSingleton: useSingleton,
        constructor: mediatorConstructor,
        id: Lavender.UuidUtils.generateUUID(),
        name: mediatorConstructor.toString()
    };
};
...
Lotus.AbstractCommand.prototype.execute = function(event) {
    ...
    this.action.addEventListener(Lavender.ActionErrorEvent.ERROR, this, "onError");
    this.action.execute();
};
...
Lotus.SkinPartList.prototype.removeItemAt = function(index) {
...
Lotus.AbstractComponent = function() {
    ...
    var _ready = false;
    var _id = Math.random();
    var _skinParts = new Lotus.SkinPartList();
    Lavender.Subject.prototype.constructor.call(this);
    this.addProperties({
        element: {
            get: function() {
...
Lotus.AbstractComponent.prototype.addAttributes = function() {
    for (var i = 0; i < this.element.attributes.length; i++) {
        ...
        if (attribute.name.indexOf("attribute-") >= 0) {
            ...
            var newProp = attribute.name.substring(index);
            var camelCased = newProp.replace(/-([a-z])/g, function(g) {
...
Lotus.AbstractCollectionView = function() {
    ...
    this.addProperties({
        ...
        collection: {
            ...
            set: function(val) {
                this.removeCollectionEventListeners();
                _collection = val;
...
Lotus.AbstractThumbnailView.prototype.getContainerSize = function() {
    var returnObj = this.thumbnailContainer !== null && this.thumbnailContainer !== undefined ? {
        width: parseInt(window.getComputedStyle(this.thumbnailContainer).width),
        height: parseInt(window.getComputedStyle(this.thumbnailContainer).height)
    } : {
        width: NaN,
        height: NaN
    };
...
Lotus.AbstractThumbnailView.prototype.resetState = function() {
    ...
    this.dispatch(new Lotus.ItemViewEvent(eventType, {
        item: this
    }));
};
...
Lotus.ImageGalleryView.prototype.onDragStart = function(event) {
    if (event.dataTransfer !== null && event.dataTransfer !== undefined) {
        ...
        try {
            ...
            event.dataTransfer.setData("text", this.model.source);
        }
    } else {
        ...
        try {
            ...
            event.originalEvent.dataTransfer.setData("text", this.model.source);
        }
    }
};
...
Lotus.Button.prototype.onClick = function(event) {
    ...
    this.dispatch(new Lavender.AbstractEvent("click", {
        target: this.buttonSkinPart,
        originalEvent: event
    }));
};
